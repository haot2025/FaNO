import models.layers_diffusion
import argparse
import json
import random
...
import os
import sys
_FANO_DIR = os.path.dirname(os.path.abspath(__file__))
if _FANO_DIR not in sys.path:
    sys.path.insert(0, _FANO_DIR)
import models
...
class MeshSpectralConv(nn.Module):
    ...
    def forward(self, x, mass, evecs, evals=None):
...
def sample_frame(paths, split, op_dir, stats, noise_std, device, rng, train=True):
    ...
    evals = torch.from_numpy(ops["evals"].astype(np.float32)).to(device)

    return x, y, mask, mass, evecs, evals
...
def rollout_metrics(model, paths, split, op_dir, stats, device, num_rollouts=10):
    ...
    for p in tqdm(paths[:num_rollouts], desc=f"rollout {split}"):
        ...
        for _ in range(gt.shape[0]):
            ...
            pred_delta_norm = model(x, mass, evecs, evals)[0]
...
# ============================================================
# Override model with FaNO DiffusionNet.
...
class MeshSpectralModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.net = models.layers_diffusion.DiffusionNet(
            C_in=c_in,
            C_out=c_out,
            C_width=width,
            num_k=k,
...
def main():
    ...
    print("[INFO] using layers_diffusion.DiffusionNet")

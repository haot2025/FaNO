...
class conv_fano(nn.Module):
    ...
    def __init__(self, C_inout, K, method='spectral', persistent_ratio=0.25, model_variant='fano'):
        ...
        if model_variant == 'fno':
            self.global_c = 0
            self.local_c = C_inout
        else:
            # Use persistent_ratio as intended:
            # global branch gets persistent_ratio of channels,
            # local spectral branch keeps the remaining channels.
            self.global_c = max(1, int(round(C_inout * persistent_ratio)))
            self.global_c = min(self.global_c, C_inout - 1)
            self.local_c = C_inout - self.global_c

        self.norm_w = nn.Parameter(
            self.scale * torch.randn(C_inout, self.local_c, K)
        )#fno baseline

        if self.global_c > 0:
            if model_variant == 'fano':
                # Normal FaNO: pooled global feature with learned channel mixing.
                self.x_w = nn.Parameter(self.scale * torch.randn(C_inout, self.global_c))
            elif model_variant == 'fano_poolnomix':
                # PoolNoMix: keep global pooling, but remove learned channel mixing.
                # Shape kept for inspection/state consistency; it is not used in forward.
                self.x_w = nn.Parameter(torch.zeros(C_inout, self.global_c), requires_grad=False)
            else:
                self.x_w = nn.Parameter(torch.zeros(C_inout, self.global_c), requires_grad=False)

            self.w1 = nn.Parameter(self.scale * torch.randn(1, K, self.global_c))
        else:
            self.x_w = nn.Parameter(torch.zeros(C_inout, 1), requires_grad=False)
            self.w1 = nn.Parameter(torch.zeros(1, K, 1), requires_grad=False)
        self.method = method # one of ['spectral', 'implicit_dense']

        

    def forward(self, x, L, mass, evals, evecs):
        # Ensure spectral basis dimension matches the model spectral weights.
        # For k_eig ablations, cached data may still contain 128 eigenvectors,
        # while self.norm_w is built with K=32/64/96.
        K_model = self.norm_w.shape[-1]
        if evals is not None and evals.shape[-1] != K_model:
            evals = evals[..., :K_model]
        if evecs is not None and evecs.shape[-1] != K_model:
            evecs = evecs[..., :K_model]
        ...
        if self.method == 'spectral':
            ...
            x_spec = to_basis(x, evecs, mass) # BLC

            if self.model_variant == 'fano':
                # Normal FaNO: global pooling + learned channel mixing.
                input_integral = torch.mean(x * mass.unsqueeze(-1), dim=1, keepdim=True)
                input_integral = torch.einsum("bxi,io->bxo", input_integral, self.x_w)

            elif self.model_variant == 'fano_poolnomix':
                # PoolNoMix: global pooling only, no learned x_w channel mixing.
                # Use the last global_c channels as the global branch.
                input_integral = torch.mean(x * mass.unsqueeze(-1), dim=1, keepdim=True)
                input_integral = input_integral[..., -self.global_c:]

            # conv
            x_c = torch.einsum("bki,iok->bko", x_spec, self.norm_w) #fno

            if self.model_variant in ['fano', 'fano_poolnomix']:
                x_spec = torch.cat([x_c, input_integral*self.w1], dim=2)
            else:
                x_spec = x_c 
...
class ChannelDropout1d(nn.Module):
    ...
    def forward(self, x):
        # [B, X, C] -> [B, C, X, 1] -> dropout -> [B, C, X, 1] -> [B, X, C]
        return self.drop(x.permute(0, 2, 1).unsqueeze(-1)).squeeze(-1).permute(0, 2, 1)
...
class FaNO(nn.Module):
    ...
    def __init__(self, C_inout, K, method='spectral', persistent_ratio=0.25):
        ...
        # global branch gets persistent_ratio of channels,
        # local spectral branch keeps the remaining channels.
        self.global_c = max(1, int(round(C_inout * self.persistent_ratio)))
        self.global_c = min(self.global_c, C_inout - 1)
        self.local_c = C_inout - self.global_c
...
class FaNO_block(nn.Module):
    ...
    def __init__(self, C_width, num_k, mlp_hidden_dims,
                 dropout=True, 
                 ...
                 model_variant='fano',
                 param_reduction=3):
        super(FaNO_block, self).__init__()
        ...
        # Diffusion block
        if isinstance(model_variant, str) and model_variant.startswith('fano'):
            # Allow model_variant to control compression ratio directly:
            #   fano_r2 -> param_reduction = 2
            #   fano_r3 -> param_reduction = 3
            #   fano_r4 -> param_reduction = 4
            local_param_reduction = param_reduction
            if model_variant.endswith('_r2'):
                local_param_reduction = 2
            elif model_variant.endswith('_r3'):
                local_param_reduction = 3
            elif model_variant.endswith('_r4'):
                local_param_reduction = 4

            # self.diffusion = S_FaNO(
            #     self.C_width,
            #     self.C_width,
            #     method=diffusion_method,
            #     n_poles=num_k,
            #     param_reduction=local_param_reduction,
            # )
            self.diffusion = FaNO(
                self.C_width,
                K=num_k,
        ...
        # MLPs
        self.MLP_C = self.C_width * 2 + (self.C_width // 4 if self.with_gradient_features else 0)
        self.mlp = MiniMLP([self.MLP_C] + self.mlp_hidden_dims + [self.C_width], dropout=self.dropout)

        self.skip = nn.Linear(C_width, C_width)
    ...
    def forward(self, x_in, mass, L, evals, evecs, gradX, gradY):
        ...
        if self.with_gradient_features:
            ...
            # feature_combined = torch.cat((x_in, x_grad_features), dim=-1)
        else:
            # Stack inputs to mlp
            feature_combined = torch.cat((x_in, x_diffuse), dim=-1)
            # feature_combined = x_in
...
class FaNO_Net(nn.Module):
    ...
    def __init__(self, C_in, C_out, C_width=128, num_k=128, N_block=4, last_activation=None, outputs_at='vertices', mlp_hidden_dims=None, dropout=True, 
                       with_gradient_features=False, with_gradient_rotations=False, diffusion_method='spectral', persistent_ratio=0.25, width_list=None, model_variant='fano', param_reduction=3):   
        """
        ...
        self.width_list = width_list if width_list is not None else [C_width] * N_block
        if len(self.width_list) != N_block:
            raise ValueError(f"width_list length {len(self.width_list)} must equal N_block {N_block}")
        ...
        # First affine layer maps input to first stage width
        self.first_lin = nn.Linear(C_in, self.width_list[0])
        self.last_lin = nn.Linear(self.width_list[-1], C_out)

        # Inter-block projections for pyramid widths
        self.proj_layers = []
        for i in range(self.N_block - 1):
            if self.width_list[i] != self.width_list[i+1]:
                proj = nn.Linear(self.width_list[i], self.width_list[i+1])
            else:
                proj = nn.Identity()
            self.proj_layers.append(proj)
            self.add_module("proj_"+str(i), self.proj_layers[-1])

        # DiffusionNet blocks
        self.blocks = []
        for i_block in range(self.N_block):
            block_width = self.width_list[i_block]
            block_mlp_hidden_dims = [block_width * 2]

            block = FaNO_block(C_width = block_width,
                                      num_k=num_k,
                                      mlp_hidden_dims = block_mlp_hidden_dims,
                                      dropout = dropout,
...
# Alias for the released FaNO network.
FaNO_model = FaNO_Net

...
class Inferencer():
    ...
    def launch(self):
        self.test_data_loader, self.test_dataset, _ = get_data_loader(
            self.params, self.params.test_path, dist.is_initialized(),
            train=False, pack=self.params.pack_data
        )
        ...
        elif self.params.model == 'fano':
            self.model = models.fano.fano(self.params).to(self.device)
        elif self.params.model == 'unet':
            self.model = models.unet.unet(self.params).to(self.device)
        elif self.params.model == 'resnet':
            self.model = models.resnet.resnet(self.params).to(self.device)
        elif self.params.model == 'lsm':
            self.model = models.lsm.lsm(self.params).to(self.device)
        elif self.params.model == 'msfno':
            self.model = models.msfno.msfno(self.params).to(self.device)
        elif self.params.model == 'deeponet':
            self.model = models.deeponet.deeponet(self.params).to(self.device)
        elif self.params.model == 'gnot':
            self.model = models.gnot.gnot(self.params).to(self.device)
        else:
    ...
    def test(self):
        ...
        num_examples = getattr(self.params, "num_vis", 6)
        num_examples = min(num_examples, len(self.test_data_loader))
        idx = list(np.random.choice(len(self.test_data_loader), size=num_examples, replace=False))
        img_idx = [np.random.randint(0, self.params.local_valid_batch_size) for _ in range(num_examples)]
        ...
        with torch.no_grad():
            for i, (inputs, targets) in enumerate(self.test_data_loader):
                ...
                self.logs['test_err'] += l2_err(u.detach(), targets.detach())
                self.logs['test_loss'] += loss.detach()

                if i in idx and ii < len(img_idx):
